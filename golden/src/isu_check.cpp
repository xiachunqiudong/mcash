...
int isu_iq_enqueue(uint64_t cycle, uint8_t bank, uint8_t cacheline_inflight, uint8_t need_linefill, uint8_t rob_id, uint8_t ch_id,
  ...
  if ((opcode & 1) == 0) {
    ...
    if (golden_rob_id != rob_id || golden_inflight != cacheline_inflight) {
      sprintf(msg_buf, "[BANK %d] isu iq enqueue check fail! GOLDEN rob id: %d inflight: %d, RTL rob id: %d inflight: %d", bank, golden_rob_id, golden_inflight, rob_id, cacheline_inflight);
      LOG_ERROR(cycle, msg_buf);
      return 1;
    }
  }
